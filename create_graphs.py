...
for j in array_size:
    ...
    for k in num_threads:
        ...
        # Store error margins
        sequential_errors.append(sequential_margin)
        pthreads_errors.append(pthreads_margin)
        openmp_errors.append(openmp_margin)

    # Create graph
    plt.figure()
    plt.errorbar(thread_counts, sequential_times, yerr=sequential_errors, fmt='o-', label='Sequential')
    plt.errorbar(thread_counts, pthreads_times, yerr=pthreads_errors, fmt='o-', label='Pthreads')
    plt.errorbar(thread_counts, openmp_times, yerr=openmp_errors, fmt='o-', label='OpenMP')
    plt.title(f'Variação do tempo em função do número de threads para grid_size = {j}')
    plt.xlabel('Número de threads')
    plt.ylabel('Tempo (s)')
    plt.legend()
    plt.savefig(f'graphs/graph_{j}.png')
    plt.close()
